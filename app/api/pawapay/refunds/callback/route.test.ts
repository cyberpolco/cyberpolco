...
function makeRequest(body: unknown) {
  return new NextRequest("http://localhost/api/pawapay/refunds/callback", {
    method: "POST",
...
describe("POST /api/pawapay/refunds/callback", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 401 when signature verification fails", async () => {
    ...
    const res = await POST(makeRequest({ refundId: "ref-1", status: "COMPLETED" }));
    expect(res.status).toBe(401);
  });

  it("upserts a refund-typed transaction and returns 200 for a valid payload", async () => {
    const res = await POST(makeRequest({ refundId: "ref-1", status: "COMPLETED" }));
    expect(res.status).toBe(200);
    expect(upsertPawaPayTransaction).toHaveBeenCalledWith(
      expect.objectContaining({ pawapayId: "ref-1", type: "refund" })
    );
  });
